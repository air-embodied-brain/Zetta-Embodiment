...
"""Freeze one single-slot native ManiSkill evolution campaign."""
...
from robots.maniskill.contracts import (  # noqa: E402
    EXECUTION_CONTRACT,
    SAFETY_LAYER,
    TASK_ID,
    TOOL_CATALOG,
...
def prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        not (repository / "robots/maniskill/run_rollout.py").is_file()
    ...
    if not args.policy_id.strip() or not 1 <= args.execute_horizon <= 50:
        raise ValueError("policy ID and a horizon in [1, 50] are required")
    config = read_json(args.env_config)
    task = TaskContract.from_dict(read_json(args.task_contract))
    task.validate_env(config)
    ...
    atomic_write_json(env_path, config, overwrite=False)
    task_path = output / "task-contract.json"
    atomic_write_json(task_path, task.as_dict(), overwrite=False)
    ...
    command = [
        str(python),
        str(repository / "robots/maniskill/run_rollout.py"),
        "--runtime-url",
        args.runtime_url,
        "--env-config",
        str(env_path),
    ...
    manifest = CampaignManifest(
        ...
        runtime={
            ...
            "task_contract_sha256": file_sha256(task_path),
            "heldout_mode": "test",
            ...
            "seed_provenance": {
                ...
                "initial_state_comparison": "strict_simulator_state_sha256",
            },
        },
    )
    ...
    preregistration = {
        ...
        "active_bundle_sha256": parent_sha,
        "success_criterion": "official info.success",
