...
from rpent.planner.provider_pool import load_provider_pool_config
            ...
            with openai_codex.Codex(config=self._build_config(mcp_url)) as codex:
                ...
                with (
                    ...
                    first_action_deadline_s = max(
                        0,
                        int(os.environ.get("RPENT_FIRST_ACTION_DEADLINE_S", "240")),
                    )
...
def _sandbox_from_env() -> Any:
    ...
    requested = os.environ.get("RPENT_CODEX_SANDBOX", "full-access").strip().lower()
    aliases = {
    ...
    try:
        ...
        raise ValueError(
            "RPENT_CODEX_SANDBOX must be read-only, workspace-write, or full-access; "
            f"got {requested!r}"
        ) from exc
