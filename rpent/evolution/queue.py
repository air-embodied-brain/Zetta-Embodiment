...
from rpent.evolution.slot_broker import EnvironmentSlotBroker, EnvironmentSlotLease
...
class SubprocessRolloutExecutor:
    ...
    def run(
        ...
        *,
        ...
        endpoint_token = "__RPENT_ENV_ENDPOINT__"
        ...
        process_environment = {
            **os.environ,
            "RPENT_POLICY_RNG_SEED": str(job.policy_rng),
        }
        if environment_endpoint is not None:
            process_environment["RPENT_ROBOCASA_ENV_ENDPOINT"] = environment_endpoint
    ...
    *,
    ...
    def get_slot_broker() -> EnvironmentSlotBroker | None:
        ...
        root = environment_slot_broker_root or os.environ.get(
            "RPENT_ENVIRONMENT_SLOT_BROKER_ROOT"
        )
        manifest = environment_ready_manifest or os.environ.get(
            "RPENT_ENVIRONMENT_READY_MANIFEST"
        )
        ...
        if active is None:
            active = _optional_positive_int(
                os.environ.get("RPENT_MAX_ACTIVE_ENVIRONMENT_SLOTS"),
    ...
    def get_provider_runtime() -> _ProviderRuntime | None:
        ...
        root = (
            ...
            or os.environ.get("RPENT_PROVIDER_ADMISSION_ROOT")
            or os.environ.get("RPENT_API_ADMISSION_ROOT")
        )
        if root is None:
            return None
        route_id = (
            provider_route_id or os.environ.get("RPENT_PROVIDER_ROUTE_ID") or "default"
        )
        legacy_limit = os.environ.get("RPENT_API_MAX_CONCURRENCY")
        initial = provider_initial_limit
        if initial is None:
            initial = _optional_positive_int(
                os.environ.get("RPENT_PROVIDER_INITIAL_CONCURRENCY") or legacy_limit,
                name="provider initial concurrency",
            )
        maximum = provider_max_limit
        if maximum is None:
            maximum = _optional_positive_int(
                os.environ.get("RPENT_PROVIDER_MAX_CONCURRENCY") or legacy_limit,
                name="provider max concurrency",
            )
        acquire_timeout = provider_acquire_timeout_s
        if acquire_timeout is None:
            acquire_timeout = _optional_positive_float(
                os.environ.get("RPENT_PROVIDER_ACQUIRE_TIMEOUT_S") or 1800.0,
                name="provider acquire timeout",
            )
        heartbeat = provider_heartbeat_s
        if heartbeat is None:
            heartbeat = _optional_positive_float(
                os.environ.get("RPENT_PROVIDER_HEARTBEAT_S") or 60.0,
    ...
    while True:
        ...
        active_worker_id = (
            worker_id
            or os.environ.get("RPENT_WORKER_ID")
