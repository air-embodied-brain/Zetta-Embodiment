...
Lives in :mod:`zetta.envs` alongside
:class:`~zetta.envs.prompt_bundle.PromptBundle` so envs
and planners can both import it without pulling in
:mod:`zetta.tools` or the RPC transport layer. Tool schemas,
handlers, server lifecycle, and the MCP allowlist live on
:class:`zetta.tools.toolkit.Toolkit` and its env subclasses —
``EnvSpec`` carries the env identity, the prompt bundle, and
the three runner hooks (``add_cli_args`` / ``parse_config`` /
``init_runtime``) that keep ``zetta/cli/main.py`` env-agnostic.
"""
...
@dataclass(frozen=True)
class EnvSpec:
    """Environment-level (non-tool) extension points for Zetta."""
