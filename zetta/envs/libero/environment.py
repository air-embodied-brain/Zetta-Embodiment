...
from zetta.envs.libero.utils import (
...
from zetta.envs.libero.vector_env import ReconfigureSubprocEnv
...
def _ensure_libero_routing() -> None:
    """Import and route the selected LIBERO variant only when an env is built."""
    package_name = "libero" if libero_type == "standard" else f"libero{libero_type}"
    core_path = f"{package_name}.{package_name}"
    core = importlib.import_module(core_path)
    if package_name != "libero":
        package = importlib.import_module(package_name)
        benchmark_module = importlib.import_module(f"{core_path}.benchmark")
        envs_module = importlib.import_module(f"{core_path}.envs")
        sys.modules["libero"] = package
        sys.modules["libero.libero"] = core
        sys.modules["libero.libero.benchmark"] = benchmark_module
        sys.modules["libero.libero.envs"] = envs_module
    _repoint_libero_config(core)


class LiberoEnv(gym.Env):
    def __init__(self, cfg, num_envs, seed_offset, total_num_processes, worker_info):
        _ensure_libero_routing()
