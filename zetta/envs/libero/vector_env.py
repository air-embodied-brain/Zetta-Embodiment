...
from zetta.envs.libero.base_vector_env import (
    BaseVectorEnv,
    CloudpickleWrapper,
    EnvWorker,
    ShArray,
...
from zetta.envs.libero.utils import get_libero_type
...
def OffScreenRenderEnv(**kwargs):
    """Construct the selected LIBERO environment without eager simulator imports."""
    import importlib

    preferred = {
        "pro": "liberopro.liberopro.envs",
        "plus": "liberoplus.liberoplus.envs",
        "standard": "libero.libero.envs",
    }.get(libero_type, "libero.libero.envs")
    candidates = (
        preferred,
        "libero.libero.envs",
        "liberopro.liberopro.envs",
        "liberoplus.liberoplus.envs",
    )
    for module_name in dict.fromkeys(candidates):
        try:
            module = importlib.import_module(module_name)
            return module.OffScreenRenderEnv(**kwargs)
        except ImportError:
            continue
    raise ImportError("No LIBERO package (standard, pro, or plus) is installed")
