    ...
    *,
...
) -> tuple[list[tuple[str, RolloutJob]], list[str]]:
    ...
    for index, seed in enumerate(manifest.rollout_seeds):
        ...
        if attempt_index >= manifest.max_infrastructure_attempts:
            # Every attempt for this logical id has been exhausted (fail-closed:
            # infrastructure failures never silently drop out of the campaign,
            # see ``EvolutionSupervisor.step()``'s ROLLOUT branch and
            # ``CandidateGateRunner.enqueue_missing``'s identical pattern for
    ...
    return round_robin_assign({manifest.task: jobs}, worker_hosts), sorted(blocked)
...
def enqueue_missing_rollouts(
    *,
    ...
    assignments, blocked = build_rollout_jobs(
