...
A thin, SDK-first backend for Zetta. ``solve()`` does four things:
...
from zetta.planner.base import (
    PlannerResult,
    add_mcp_prefix,
    strip_mcp_prefix,
)
...
class ClaudeCodePlanner:
    ...
    def _build_options(self, sdk: Any, *, toolkit: Toolkit, max_turns: int) -> Any:
        ...
        return sdk.ClaudeAgentOptions(
            ...
            mcp_servers={
                "zetta": _build_zetta_server(
                    sdk,
                    toolkit=toolkit,
                ),
            },
            add_dirs=[self._output_dir, *self._extra_dirs],
            # Ignore user/project .claude configuration; Zetta owns the loop.
            setting_sources=[],
...
# ---------------------------------------------------------------------------
# Tool bridge (Zetta registry -> SDK MCP server)
# ---------------------------------------------------------------------------


def _build_zetta_server(sdk: Any, *, toolkit: Toolkit) -> Any:
    sdk_tools = []
