...
def write_text_file(path: str, content: str) -> dict:
    p = _resolve(path)
    if os.environ.get("ZETTA_EPISODE_MEMORY_FROZEN", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
        ...
        if not target.is_relative_to(output_root):
            return {
                "error": (
                    "episode memory is frozen; write_text_file is restricted to "
                    f"the current output_dir ({output_root})"
                ),
                "path": str(target),
            }
...
def finish(status: str, summary: str) -> dict:
    ...
    :meth:`zetta.planner.api_loop.ApiAgentLoop._solve` and the
    ``pending_finish`` bookkeeping in
    :class:`zetta.planner.claude_code._Recorder`.
    """
